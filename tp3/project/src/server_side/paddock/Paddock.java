 ...
 */
public class Paddock implements Paddock_Itf {
     ...
     */
    public Paddock(Log_Interface log, int horses_total, int spec_total, String rmiRegHostName, int rmiRegPortNumber){
     ...
     */
    @Override
    public void announceNextRace(VectorClock vc){
        rl.lock();
        try{
     ...
     */
    @Override
    public boolean checkSpecCount(VectorClock vc){
        boolean b = false;
        rl.lock();
        try {
     ...
     */
    @Override
    public void acceptTheBets(VectorClock vc) throws RemoteException{
        rl.lock();
        try{
...
//            broker.broker_states = MyThreadBroker.Broker_States.WAITING_FOR_BETS;
    ...
    /**
     * Broker shutdown Paddock monitor.
     */
    @Override
    public void terminate() throws RemoteException {
        rl.lock();
        try{
            //Paddock_Exec.shutdown();
            Register reg  = null;
            ...
            String nameEntryObj = RegistryConfig.paddockNameEntry;
            ...
            System.out.print("Paddock module shutting down...\n");
     ...
     */
    @Override
    public void proceedToStartLine(int id, VectorClock vc) throws RemoteException{
        rl.lock();
        try{
...
//                horse.horse_states = MyThreadHorses.Horse_States.AT_THE_START_LINE;
     ...
     */
    @Override
    public void proceedToPaddock(int id, int agi, VectorClock vc) throws RemoteException{
        double totalagi;
        double tmp;
        rl.lock();
        try {
     ...
     */
    @Override
    public boolean checkForSpectators(VectorClock vc) {
        boolean b = false;
        rl.lock();
        try {
            if(spec_count==spec_total)
...
//                spectator.spec_states = MyThreadSpec.Spec_States.APPRAISING_THE_HORSES;
     ...
     */
    @Override
    public int[] appraiseHorses(int spec_id, VectorClock vc){
        int[] a = null;
        rl.lock();
        try{
            System.out.print("S" + spec_id + " : Appraising the Horses(Horse_Count = " + appraise_count + ")\n");
            a=horses_agi;
            appraise_count++;
            if(appraise_count == horses_total){
                ...
                System.out.print("S" + spec_id + " : Horses are being appraised (Horse_count = " + appraise_count + ") -> All Horses have been apraised!\n");
            }
            else{
                System.out.println("S" + spec_id + " : Horses are being appraised (Horse_count = " + appraise_count + ")");
            }
        }finally{
            rl.unlock();
        }
     ...
     */
    @Override
    public void goPlaceABet(int id, VectorClock vc) throws RemoteException{
        rl.lock();
        try{           
            spec_count--;
